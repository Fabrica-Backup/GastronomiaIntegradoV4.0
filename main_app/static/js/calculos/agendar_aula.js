function calculos() {
    ...
    if (typeof jsonReceitaIngrediente === 'undefined') {
        ...
        for (i = 0; i < receitaArr.length; i++) {
            $.map(jsonReceitaIngrediente, function(valReceitaIngrediente) {
                if (valReceitaIngrediente.id_receita == receitaArr[i]) {
                    ...
                    var qtdIngrediente = calculaIngredientes(qtdIngrediente_receita);
    ...
    function calculaIngredientes(qtdIngrediente_receita) {
        // var serialQtdReceita = $('#reserva').serializeArray();

        for (var i = 0; i < rec; i++) {
            var qtdReceita = $('.qtdReceita' + i + '').val();
            console.log(qtdReceita, qtdIngrediente_receita);
            return qtdReceita * qtdIngrediente_receita;
        }
        // pega a quantidade da receita especifica para ser feito na aula
